...
inputOne :: String -> (String,Int,String)
inputOne s = (name,sector,check)
  where name = init $ reverse $ dropWhile (/='-') $ reverse s
        sector = read $ reverse $ takeWhile (/='-') $ tail $ dropWhile (/='[') $ reverse s
        check = tail $ init $ dropWhile (/='[') s

input :: [(String,Int,String)]
input = map inputOne $ lines rawInput

incr :: Ord a => a -> Map a Int -> Map a Int
incr = alter (Just . maybe 1 (+1))

count :: Ord a => [a] -> Map a Int
count = foldr incr Data.Map.empty

isValid :: (String,Int,String) -> Bool
isValid (s,_,c) = all (uncurry (==)) $
                  zip c $
                  map fst $
                  sortOn (\(l,n) -> (-n,l)) $
                  toList $
...
solution1 :: Int
solution1 = sum $ map (\(_,i,_) -> i) $ filter isValid input

shift :: Int -> Char -> Char
shift _ '-' = '-'
shift i c = chr ((((ord c - 97) + i) `mod` 26) + 97)

solution2 :: Int
solution2 = fst $
            head $
            filter (isInfixOf "northpole-object-storage" . snd) $
            map (\(s,i,_) -> (i, map (shift i) s)) $
