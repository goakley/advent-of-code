...
readLine :: String -> [Either String String]
readLine ""       = []
readLine ('[':xs) = Right (takeWhile (/=']') xs) : readLine (dropWhile (/=']') xs)
readLine (']':xs) = readLine xs
readLine xs       = Left (takeWhile (/='[') xs) : readLine (dropWhile (/='[') xs)

input :: [[Either String String]]
input = map readLine $ lines rawInput

hasABBA :: String -> Bool
hasABBA (a:b:c:d:xs) = (b == c && a == d && a /= b) ||
                       hasABBA (b:c:d:xs)
hasABBA _            = False

isTLS :: [Either String String] -> Bool
isTLS xs = any hasABBA supernets && all (not . hasABBA) hypernets
  where supernets = lefts xs
        hypernets = rights xs

solution1 :: Int
solution1 = length $ filter isTLS input

getABAs :: String -> [(Char,Char,Char)]
getABAs (a:b:c:xs) = (if a == c && a /= b then ((a,b,c):) else id) (getABAs (b:c:xs))
getABAs _          = []

isSSL :: [Either String String] -> Bool
isSSL xs = any (\(a,b,_) -> any ([b,a,b] `isInfixOf`) hypernets) $ concatMap getABAs supernets
  where supernets = lefts xs
        hypernets = rights xs

solution2 :: Int
solution2 = length $ filter isSSL input
