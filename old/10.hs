import Data.Map (Map, alter, empty, insert, lookup, member, toList)
...
type Bot = (Maybe Int,Maybe Int)

data Instruction = Input Value Int | Give Int (Bool,Value) (Bool,Value)

readLine :: String -> Instruction
readLine s = case (valueMatch,botMatch) of
              ([[_,v,b]],[]) -> Input (read v) (read b)
              ([],[[_,b,lt,l,ht,h]]) -> Give (read b) (lt == "bot",read l) (ht == "bot",read h)
              _ -> undefined
  where valueMatch = s =~ "^value ([0-9]+) goes to bot ([0-9]+)$"
        botMatch = s =~ "^bot ([0-9]+) gives low to (bot|output) ([0-9]+) and high to (bot|output) ([0-9]+)$"

input :: [Instruction]
input = map readLine $ lines rawInput

updateBot :: Int -> Bot -> Bot
updateBot i (Nothing,Nothing) = (Just i,Nothing)
updateBot i (Just j,Nothing) = (Just (min i j),Just (max i j))
updateBot i (Nothing,Just j) = updateBot i (Just j,Nothing)
updateBot _ _ = undefined

build :: [Instruction] -> (Map Int Bot,Map Int Value) -> (Map Int Bot,Map Int Value)
build [] m = m
build (Input v b : xs) (m,n) = build xs (m',n)
  where m' = alter (Just . updateBot v . fromMaybe (Nothing,Nothing)) b m
build (Give b (i,x) (j,y) : xs) (m,n) = case Data.Map.lookup b m of
                                         (Just (Just l,Just h)) -> build xs (recurse l h)
                                         _ -> build (xs ++ [Give b (i,x) (j,y)]) (m,n)
  where recurse l h = case (i,j) of
                       (True,True) -> (fst (build [Input l x, Input h y] (m,n)),n)
                       (True,False) -> (fst (build [Input l x] (m,n)),if member y n then undefined else insert y h n)
                       (False,True) -> (fst (build [Input h y] (m,n)),if member x n then undefined else insert x l n)
                       (False,False) -> (m,if member x n then undefined else insert x l (if member y n then undefined else insert y h n))

bots :: Map Int Bot
bots = fst $ build input (empty,empty)

outputs :: Map Int Value
outputs = snd $ build input (empty,empty)

solution1 :: Int
solution1 = fst $ head $ filter ((==(Just 17,Just 61)) . snd) $ toList bots

solution2 :: Int
solution2 = product $ map snd $ filter (\(n,_) -> n `elem` [0,1,2]) $ toList outputs
